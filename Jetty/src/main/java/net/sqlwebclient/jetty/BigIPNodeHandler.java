...
final class BigIPNodeHandler extends AbstractHandler {
    ...
    public static String check(int port) {
        return H.GET("http://localhost:" + port + NODE_HANDLER_PATH).body;
    }

    public static String online(int port, String secretToken) {
        return H.POST("http://localhost:" + port + NODE_HANDLER_PATH + "?secret=" + secretToken + "&state=online").body;
    }

    public static String offline(int port, String secretToken) {
        return H.POST("http://localhost:" + port + NODE_HANDLER_PATH + "?secret=" + secretToken + "&state=offline").body;
    }
}
