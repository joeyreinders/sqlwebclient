...
final class ShutdownHandler extends AbstractHandler {
    private final Server server;
    private final WebAppContext context;
    private final String secret;

    ShutdownHandler(Server server, WebAppContext context, String secret) {
        this.server = server;
    ...
    public static String shutdown(int port, String shutdownSecret) {
        return H.POST("http://127.0.0.1:" + port + "/shutdown?secret=" + shutdownSecret).body;
    }
}